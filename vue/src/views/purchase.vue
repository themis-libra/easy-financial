<template>
    <div class="purchase">
        ...
        <el-card class="purchase_card"  v-else-if="buyState === 1">
            <div class="card_header">产品购买</div>
            ...
            <div class="business_info">
                ...
                <div class="info_content">
                    <div class="input_item">
                        <span>产品代码</span>
                        <el-input placeholder="请输入产品代码" v-model="businessInfo.spid" class="input"></el-input>
                    </div>
                    <div class="input_item">
                        <span>购买金额</span>
                        <el-input placeholder="请输入购买金额" v-model="businessInfo.samount" class="input" style="width: 400px"></el-input>
                    </div>
                    <div class="bank_card_item" style="display: flex">
                        <div class="input_item">
                            <span>资金方式</span>
                            <el-select placeholder="请选择资金方式" v-model="businessInfo.sbankcardNumber" class="input" style="width: 400px">
                                <el-option
                    ...
                    <div class="add_button" style="margin-left: 43%; margin-top: 20px">
                        <el-button type="primary" icon="el-icon-circle-plus-outline" @click="confirmAdd">确认添加</el-button>
                    </div>
                </div>
            </div>
        </el-card>
        <el-card class="purchase_success" v-else-if="buyState === 2">
            <div class="success">
                ...
                <div class="success_info">申购成功！</div>
                <div class="info_name">投资人名称：{{ this.businessInfo.scname}}</div>
                <div class="info_name">交易账号：{{this.userAccount}}</div>
                <div class="business_info">
                    <div class="business_pro">
                        <span style="font-weight: bold; font-size: 18px">产品代码</span>
                        <span>{{ this.businessInfo.spid }}</span>
                    </div>
                    <div class="business_count">
                        <span style="font-weight: bold; font-size: 18px">购买金额</span>
                        <span>{{ this.businessInfo.samount }}</span>   
                    </div>
                </div>
                <div class="button">
                    <el-button type="primary" @click="()=>{this.$router.push({name: 'PruchaseSearch'})}">申购详情</el-button>
...
export default {
    ...
    data () {
        return {
            ...
            wayOptions: [],
            buyState: 1, // 0表示当前不在申购时间 1表示在申购界面 2表示申购成功界面
        }
    },
    methods: {
        finish() {
        ...
        confirmAdd() {
            this.businessInfo.scname = this.userInfo.cname
            this.businessInfo.scid = this.userInfo.cid
            this.businessInfo.rid = "1"
            axios.post('http://127.0.0.1:8080/subscribe/buy', this.businessInfo)
                .then(res => {
                    if (res.code === 200) {
                        this.$message.success("申购产品成功")
                        this.buyState = 2
                    }
                })
        },
